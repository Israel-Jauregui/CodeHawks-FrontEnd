...
def validate_snapshot(snapshot: dict[str, Any], config: Config) -> dict[str, Any]:
    ...
    if live_mx and live_mx != expected_mx:
        raise PreflightError("Conflicting or non-Cloudflare apex MX records detected")
    if live_spf and live_spf != expected_spf:
        raise PreflightError("The apex SPF record conflicts with Cloudflare Email Routing")
    ...
    if settings_enabled and (not expected_mx or len(expected_spf) != 1):
        raise PreflightError(
            "Cloudflare did not return the required Email Routing MX/SPF record set"
        )
    missing_routing_records = expected_routing_keys - live_dns_keys
    if settings_enabled and missing_routing_records:
        raise PreflightError(
    ...
    return {
        ...
        "routing_dns_records": len(expected_routing_keys),
